...
FailAll = function()
	if ObjectiveStealCodes ~= nil and not USSR.IsObjectiveCompleted(ObjectiveStealCodes) then
		USSR.MarkFailedObjective(ObjectiveStealCodes)
	end
	if ObjectiveDestroyTemple ~= nil and not USSR.IsObjectiveCompleted(ObjectiveDestroyTemple) then
		USSR.MarkFailedObjective(ObjectiveDestroyTemple)
	end
